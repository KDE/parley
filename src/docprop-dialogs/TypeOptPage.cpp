...
kDebug() << "Setup special combobox: " << wordType << subType;
    ...
    if ( subType.isEmpty() ) {
        ...
        specialTypeComboBox->addItem( i18nc("@item:inlistbox selection for word types with special meaning in the practice (like noun for article and verb for conjugation) - no special word type","None") );
