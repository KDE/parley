    ...
    , m_config{config}
{
    ...
    addPage(m_generalPracticeOptions,
            ...
            QStringLiteral("general-setup"),
            i18nc("Configure general settings for practicing vocabulary.", "General Practice Settings"),
            true);

    m_blockOptions = new BlockOptions(this);
    addPage(m_blockOptions,
            i18nc("@title:group vocabulary can be set to be blocked for a certain amount of time", "Blocking"),
            QStringLiteral("cards-block"),
            i18n("Blocking Settings"),
            true);

    m_thresholdOptions = new ThresholdOptions(doc, this);
    addPage(m_thresholdOptions,
            i18nc("@title:group ignore vocabulary based on some properties like word type", "Thresholds"),
            QStringLiteral("practice-setup"),
            i18n("Threshold Settings"),
            true);

    m_specificPracticeOptions = new SpecificPracticeOptions(this);
    addPage(m_specificPracticeOptions,
            i18nc("@title:group Configure specific settings for practicing vocabulary, short title in config dialog.", "Specific"),
            QStringLiteral("specific-setup"),
            i18nc("Configure specific settings for practicing vocabulary.", "Specific Practice Settings"),
            true);
...
{
    return m_thresholdOptions->hasChanged() || m_blockOptions->hasChanged();
}

bool ConfigurePracticeDialog::isDefault()
{
    return m_thresholdOptions->isDefault() && m_blockOptions->isDefault();
}
